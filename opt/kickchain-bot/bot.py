...
LOG_INCOMING_MESSAGES = parse_bool_env("LOG_INCOMING_MESSAGES", True)
...
def normalize_group_chat_id(value: int) -> str:
    """
    Normalize Telegram group IDs for tolerant matching.
    Supports values written with or without the -100 prefix.
    """
...
def group_id_matches(chat_id: int, configured_group_id: int) -> bool:
    if not configured_group_id:
        return True
    if chat_id == configured_group_id:
        return True
    return normalize_group_chat_id(chat_id) == normalize_group_chat_id(configured_group_id)
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if LOG_INCOMING_MESSAGES:
        logger.info(
            "[INCOMING] chat_id=%s type=%s user_id=%s text=%r",
            msg.chat.id,
            msg.chat.type,
    ...
    if msg.chat.type not in {"group", "supergroup"}:
        logger.debug("[SKIP] Non-group message: type=%s", msg.chat.type)
        return
    if STRICT_GROUP_ID_FILTER and GROUP_ID and not group_id_matches(msg.chat.id, GROUP_ID):
        logger.info(
            "[SKIP] Group ID filter mismatch: incoming=%s configured=%s",
            msg.chat.id,
            GROUP_ID,
        )
        return
    ...
    user = msg.from_user
    if not user:
        logger.debug("[SKIP] Missing from_user")
        return

    raw_text = msg.text or msg.caption or ""
    text = raw_text.strip()
    if not text:
        logger.debug("[SKIP] Empty text/caption")
        return
    user_id = user.id
    ...
    if user.is_bot or text.startswith("/"):
        logger.debug("[SKIP] Bot/command message")
        return
    ...
    category = detect_signal(text)
    if not category:
        logger.debug("[SKIP] No signal detected")
        return
...
def main():
    ...
    logger.info(
        "[BOOT] GROUP_ID=%s STRICT_GROUP_ID_FILTER=%s SAVE_ALL_GROUP_TO_MEMORY=%s LOG_INCOMING_MESSAGES=%s",
        GROUP_ID,
